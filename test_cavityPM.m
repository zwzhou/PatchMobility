...
[ Ysam,Ydif, Zsam,Zdif ] = f_cavityPM( lx,ly,lz, nx,ny, rho,cc, freq, Nmodal,np,nq,nr );


fac = 1 ;
figure(1)
impedance = squeeze(Zsam(42,72,:));
plot(freq, 10*log10(abs(impedance)/fac),'b')
hold on
impedance2 = squeeze(Zsam(42,42,:));
plot(freq, 10*log10(abs(impedance2)/fac),'m')
legend('transfer impedance','input impedance')
xlabel('Frequency (HZ)');
ylabel('Magnitude (dB)');

figure(2)
patchmobility = squeeze(Ysam(42,72,:));
plot(freq,20*log10(abs(patchmobility) ),'b')
hold on
patchmobility2 = squeeze(Ysam(42,42,:));
